...
public class BakedModelBlockBranchMangrove implements IBakedModel {
	...
	private IBakedModel sleeves[][] = new IBakedModel[6][4];
	private IBakedModel cores[][] = new IBakedModel[3][4]; //4 Cores for 3 axis with the bark texture and all 6 sides rotated appropriately.
	private IBakedModel rings[] = new IBakedModel[4]; //4 Cores with the ring textures on all 6 sides
	...
	public List<BakedQuad> getQuads(IBlockState blockState, EnumFacing side, long rand) {
		...
		if (blockState instanceof IExtendedBlockState) {
			int coreRadius = MathHelper.clamp(((BlockBranch) blockState.getBlock()).getRadius(blockState), 1, 4);
	...
	protected int[] pollConnections(int coreRadius, IExtendedBlockState extendedBlockState) {
		...
		for(EnumFacing dir: EnumFacing.VALUES) {
			...
			connection = connection > 4 ? 0 : connection;
